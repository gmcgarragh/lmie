...
__global__ void kernel(int i_qsiz, int n_qsiz, int n_derivs, FLOAT lambda,
      ...
      *
      *-----------------------------------------------------------------------*/
     i = threadIdx.x * kernel_work_n(n_derivs, n1, n2);
...
*/
     ...
     for (j = 1; j <= n1; ++j) {
          ...
          for (k = 0; k < n_derivs; ++k) {
               c6 = D_l[A2(j,k)] / m - c4 * m_l[k] + a2_l[k];
          ...
          for (k = 0; k < n_derivs; ++k) {
               c4 = m_l[k] * D[j] + m * D_l[A2(j,k)] + a2_l[k];
...
int lmie_core_cuda(int n_qsiz, int n_derivs,
      ...
      *
      *-----------------------------------------------------------------------*/
     work_size = THREADSPERBLOCK * kernel_work_n(n_derivs, n1, n2);

     fprintf(stderr, "%d %d %d %d %d %d %d\n", n_qsiz, n1, n2, n_qang, blocksPerGrid, threadsPerBlock, work_size);

     kernel<<<blocksPerGrid, threadsPerBlock, work_size>>>(0, n_qsiz, n_derivs, lambda, h_m, d_m_l, h_m2, d_qx, d_qw, d_nr, d_fv1, d_fv2, d_fv3, d_fv4, d_qx2, d_qw2, d_cext, d_csca, d_cbak, d_g, d_pf, d_qx_l, d_qw_l, d_nr_l, d_cext_l, d_csca_l, d_cbak_l, d_g_l, d_pf_l, n1, n2, n_qang);
