...
int phase_func_integ_gc(int n_coef, int n_quad, double **pf,
     ...
     if (! symmetric) {
          ...
          for (i = 0; i < n_quad / 2; ++i) {
               ...
               a = 1.;
               for (j = 0; j < n_coef; ++j) {
                    chi[0][j] += P[0][j] * (b1[0] * a + b2[0]);
                    chi[1][j] += P[3][j] *  b1[1] * a + P[2][j] * b2[1];
                    chi[2][j] += P[2][j] *  b1[2] * a + P[3][j] * b2[2];
                    chi[3][j] += P[0][j] * (b1[3] * a + b2[3]);
                    chi[4][j] += P[1][j] * (b1[4] * a + b2[4]);
                    chi[5][j] += P[1][j] * (b1[5] * a + b2[5]);
...
int phase_func_integ_gc2(int n_coef, int n_quad, double **pf,
     ...
     if (! symmetric) {
          for (i = 0; i < n_coef; ++i) {
               for (j = 0; j < n_quad; ++j) {
                    chi[0][i] += P[0][j][i] *  pf[0][j] * qw[j];
                    chi[1][i] += P[2][j][i] * (pf[1][j] + pf[2][j]) * qw[j];
                    chi[2][i] += P[3][j][i] * (pf[1][j] - pf[2][j]) * qw[j];
                    chi[3][i] += P[0][j][i] *  pf[3][j] * qw[j];
                    chi[4][i] += P[1][j][i] *  pf[4][j] * qw[j];
                    chi[5][i] += P[1][j][i] *  pf[5][j] * qw[j];
               }
          ...
          if (n_quad % 2 != 0) {
               ...
               for (j = 0; j < n_coef; ++j) {
                    ...
                    chi[0][j] += P[0][i][j] *  pf[0][i] * qw[i];
                    chi[1][j] += P[2][i][j] * (pf[1][i] + pf[2][i]) * qw[i];
                    chi[2][j] += P[3][i][j] * (pf[1][i] - pf[2][i]) * qw[i];
                    chi[3][j] += P[0][i][j] *  pf[3][i] * qw[i];
                    chi[4][j] += P[1][i][j] *  pf[4][i] * qw[i];
                    chi[5][j] += P[1][i][j] *  pf[5][i] * qw[i];
               }
          }
          ...
          for (i = 0; i < n_coef; ++i) {
               ...
               for (j = 0; j < n_quad / 2; ++j) {
                    ...
                    chi[0][i] +=  P[0][j][i] * (pf[0][jj1] * a + pf[0][jj2])  * qw[jj2];
                    chi[1][i] += (P[3][j][i] * (pf[1][jj1]     + pf[2][jj1])  * a +
                                  P[2][j][i] * (pf[1][jj2]     + pf[2][jj2])) * qw[jj2];
                    chi[2][i] += (P[2][j][i] * (pf[1][jj1]     - pf[2][jj1])  * a +
                                  P[3][j][i] * (pf[1][jj2]     - pf[2][jj2])) * qw[jj2];
                    chi[3][i] +=  P[0][j][i] * (pf[3][jj1] * a + pf[3][jj2])  * qw[jj2];
                    chi[4][i] +=  P[1][j][i] * (pf[4][jj1] * a + pf[4][jj2])  * qw[jj2];
                    chi[5][i] +=  P[1][j][i] * (pf[5][jj1] * a + pf[5][jj2])  * qw[jj2];
...
static void phase_angle_exp_gc(int n_elem, int n_coef, int i,
                               double **chi, double *p00, double *p0p2,
     ...
     for ( ; j < n_coef; ++j) {
          ...
          if (n_elem > 1) {
               ...
               a[1] += (chi[1][j] + chi[2][j]) * p2p2[j];
               a[2] += (chi[1][j] - chi[2][j]) * p2m2[j];
          }
     }
...
void phase_func_exp_gc(double **chi, int n_coef,
     ...
     for (i = 0; i < n_pf; ++i) {
          ...
          phase_angle_exp_gc(n_elem, n_coef, i, chi, p00, p0p2, p2p2, p2m2, pf);
     }
...
void phase_func_exp_gc_sym(double **chi, int n_coef,
     ...
     for (i1 = 0; i1 < n_pf2; ++i1) {
          ...
          phase_angle_exp_gc(n_elem, n_coef, i1, chi, p00, p0p2, p2p2, p2m2, pf);
          ...
          phase_angle_exp_gc(n_elem, n_coef, i2, chi, p00, p0p2, p2m2, p2p2, pf);
     }
...
void phase_func_exp_lc(double **chi, int n_coef,
     ...
     for (i = 0; i < n_pf; ++i) {
          ...
          phase_angle_exp_lc(n_elem, n_coef, i, chi, P_l, pf);
     }
...
void phase_func_exp_lc_sym(double **chi, int n_coef,
     ...
     for (i1 = 0; i1 < n_pf2; ++i1) {
          ...
          phase_angle_exp_lc(n_elem, n_coef, i1, chi, P_l, pf);
          ...
          phase_angle_exp_lc(n_elem, n_coef, i2, chi, P_l, pf);
     }
